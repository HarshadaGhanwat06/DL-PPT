...
def compute_metrics(y_true_avc_space: np.ndarray, y_pred_avc_space: np.ndarray) -> Dict[str, float]:
    errors = y_pred_avc_space - y_true_avc_space
...
def ensure_output_paths_available(config: DualBranchAdvancedConfig) -> Dict[str, Path]:
    ...
    if existing:
        raise FileExistsError(
            "Refusing to overwrite existing cnn_dual_advanced outputs:\n" + "\n".join(existing)
        )
    return output_paths
