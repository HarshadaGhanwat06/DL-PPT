...
def build_model() -> ImprovedCNN:
    return ImprovedCNN(input_channels=2)
...
def plot_results(
    ...
    loss_curve_path = plots_dir / "cnn_improved_loss_curve.png"
    val_mae_curve_path = plots_dir / "cnn_improved_val_mae_curve.png"
    predicted_vs_true_path = plots_dir / "cnn_improved_predicted_vs_true.png"
    error_histogram_path = plots_dir / "cnn_improved_error_histogram.png"

    plt.figure(figsize=(8, 5))
    plt.plot(history["train_loss"], linewidth=2)
    plt.xlabel("Epoch")
    plt.ylabel("SmoothL1 Loss")
    plt.title("Improved CNN Training Loss")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    ...
    plt.title("Improved CNN Validation MAE")
    plt.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(val_mae_curve_path, dpi=200)
    plt.close()
    ...
    for index, axis in enumerate(axes):
        ...
        axis.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(predicted_vs_true_path, dpi=200)
    plt.close(fig)
    ...
    for index, axis in enumerate(axes):
        ...
        axis.grid(alpha=0.3)
    fig.tight_layout()
    fig.savefig(error_histogram_path, dpi=200)
    plt.close(fig)

    return {
        "loss_curve_path": str(loss_curve_path),
        "val_mae_curve_path": str(val_mae_curve_path),
        "predicted_vs_true_path": str(predicted_vs_true_path),
...
def train_and_evaluate(config: ImprovedCNNConfig) -> Dict[str, object]:
    ...
    plot_paths = plot_results(history, test_result["y_true"], test_result["y_pred"], config.plots_dir)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train the improved CNN regressor for PEP and AVC.")
